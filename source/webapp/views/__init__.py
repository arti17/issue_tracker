from .issue_views import IndexView, IssueView, IssueCreateView, IssueUpdateView, IssueDeleteView
from .status_views import StatusView, StatusCreateView, StatusUpdateView, StatusDeleteView
from .type_views import TypeView, TypeCreateView, TypeUpdateView, TypeDeleteView
from .project_views import ProjectView, ProjectDetailView, ProjectCreateView, ProjectDeleteView, ProjectUpdateView, ProjectCreateIssueView, AddProjectUsers, DeleteProjectUser
from .base_views import DetailView
